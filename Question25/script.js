    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "B") {
				feedback.textContent = "✔ Correct! The queue follows **FIFO (First In, First Out)** order, so the new element (6) is added to the end of the queue, making it **7,9,8,6**.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is B: 7,9,8,6**. In a queue, new elements are always added to the **rear**.";
				feedback.className = "wrong";
			}
        });
    } else {

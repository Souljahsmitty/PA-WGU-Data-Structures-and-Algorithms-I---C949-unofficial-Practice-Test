    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "B") {
				feedback.textContent = "✔ Correct! A **hash table** uses an **array** as the primary storage format, where each index in the array corresponds to a key-value pair.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is **B. Array**. Hash tables use an **array** for storing key-value pairs, ensuring fast lookups.";
				feedback.className = "wrong";
			}
        });
    } else {

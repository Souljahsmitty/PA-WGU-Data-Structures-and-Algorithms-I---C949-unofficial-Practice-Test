    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "D") {
				feedback.textContent = "✔ Correct! The **height** of this tree is **two** because there are **two edges** from the root to the deepest leaf.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is **D. Two**. The height of a tree is measured by counting the longest path from the root to a leaf node.";
				feedback.className = "wrong";
			}
        });
    } else {

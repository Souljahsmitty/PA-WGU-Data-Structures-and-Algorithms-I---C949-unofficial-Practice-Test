    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "C") {
				feedback.textContent = "✔ Correct! The **peek()** operation returns the **top element of the stack**, which is **8**.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is **C. 8**. The **peek()** function does not remove elements but simply views the top.";
				feedback.className = "wrong";
			}
        });
    } else {

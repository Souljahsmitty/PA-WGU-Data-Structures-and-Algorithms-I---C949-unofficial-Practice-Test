    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "D") {
				feedback.textContent = "✔ Correct! The midpoint in quicksort is calculated as (lowIndex + highIndex) / 2, which gives (5 + 9) / 2 = 7.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is D: The midpoint is calculated using (lowIndex + highIndex) / 2, resulting in 7.";
				feedback.className = "wrong";
			}
        });
    } else {

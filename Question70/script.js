document.addEventListener("DOMContentLoaded", function() {
    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "B") {
				feedback.textContent = "✔ Correct! The '+= ' operator is an assignment operator that adds a value to a variable and reassigns it.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is B: The '+= ' operator updates a variable by adding a value to it.";
				feedback.className = "wrong";
			}
        });
    } else {
    ...
    if (backButton) {
        backButton.addEventListener("click", function() {
            ...
            window.location.href = "../Question69/index.html";
        });
    } else {
        console.error("backQuestion button not found!");
    }

    const nextButton = document.getElementById("resultQuestion");
    if (nextButton) {

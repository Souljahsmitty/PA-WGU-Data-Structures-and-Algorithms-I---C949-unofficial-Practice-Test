    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "B") {
				feedback.textContent = "✔ Correct! The **dequeue operation** removes the front element, which is **7**, leaving 9 as the new front of the queue.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is B: The correct answer is **B. 9**. The queue follows FIFO, so 7 is removed, and 9 becomes the new front.";
				feedback.className = "wrong";
			}
        });
    } else {

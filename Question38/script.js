    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "C") {
				feedback.textContent = "✔ Correct! In a **stack**, the `push(1)` function adds **1 to the top**. This makes the new stack **1,8,9,3,5**.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. In a **stack**, the `push(1)` function adds **1 to the top**. This makes the new stack **1,8,9,3,5**.";
				feedback.className = "wrong";
			}
        });
    } else {

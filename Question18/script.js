    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "A") {
				feedback.textContent = "✔ Correct! When an `if` statement is placed inside another `if` statement, it is called a **nested if statement**.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is A. A **nested if statement** is an `if` statement inside another `if` statement, allowing conditional logic within different conditions.";
				feedback.className = "wrong";
			}
        });
    } else {

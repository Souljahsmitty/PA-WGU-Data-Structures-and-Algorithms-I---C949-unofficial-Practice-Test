    ...
    if (submitButton) {
        submitButton.addEventListener("click", function() {
			...
			if (selectedAnswer.value === "B") {
				feedback.textContent = "✔ Correct! Since `x = 28`, the second condition `elif x <= 30` is true, so `x` is assigned the value `25`.";
				feedback.className = "correct";
			} else {
				feedback.textContent = "✘ Incorrect. The correct answer is B: Since `x = 28`, the second condition `elif x <= 30` is true, so `x` is assigned the value `25`.";
				feedback.className = "wrong";
			}
        });
    } else {
